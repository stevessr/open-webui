...
class Pipe:
    ...
    class UserValves(BaseModel):
        ...
        ENABLE_WEB_SEARCH: bool = Field(
            default=True, description="开启 Web Search (互联网搜索)"
        )
        ENABLE_X_SEARCH: bool = Field(
            default=True, description="开启 X Search (X/Twitter 帖子搜索)"
        )
        ENABLE_CODE_INTERPRETER: bool = Field(
            default=True, description="开启 Code Interpreter (代码解释器)"
        )

        # --- 高级工具配置 ---
        FILE_SEARCH_COLLECTION_IDS: str = Field(
            default="", description="File Search 集合 ID (逗号分隔，例如 id1,id2)"
        )
        MCP_SERVER_URL: str = Field(
            default="", description="MCP Server URL (例如 https://your-mcp-server)"
        )

        # --- 调试与参数 ---
        DEBUG_ERROR: bool = Field(
            default=True, description="若 API 报错，在对话框显示详细 JSON 错误信息"
        )
        TEMPERATURE: float = Field(
            default=0.6, ge=0.0, le=2.0, description="Temperature (0.0-2.0)"
        )
        MAX_TOKENS: int = Field(
            default=4096, ge=1, le=32768, description="Max Tokens (1-32768)"
        )
        API_KEY: str = Field(default="", description="API Key (sk-...)")
        MODEL_ID: str = Field(
        ...
        CHAT_TYPE: Literal["responses", "chat/completions"] = Field(
            default="chat/completions",
            description="Chat Endpoint Type",
        )
    ...
    def pipe(
        ...
        api_key = self.uservalues.API_KEY
        if not api_key:
            return (
                "Error: OPENROUTER_API_KEY not set." + self.uservalues.model_dump_json()
            )
        ...
        payload = {
            "model": self.uservalues.MODEL_ID,
            "messages": messages,
            "stream": True,
        ...
        if self.uservalues.FILE_SEARCH_COLLECTION_IDS:
            ids = [
                x.strip()
                for x in self.uservalues.FILE_SEARCH_COLLECTION_IDS.split(",")
                if x.strip()
            ]
            if ids:
                xai_tools.append({"type": "file_search", "collection_ids": ids})

        # 处理 MCP
        if self.uservalues.MCP_SERVER_URL:
            xai_tools.append(
                {"type": "mcp", "server_url": self.uservalues.MCP_SERVER_URL}
            )
        ...
        url = f"{self.uservalues.BASE_URL}/{self.uservalues.CHAT_TYPE}"
