...
def load_oauth_providers():
    ...
    if OAUTH_CLIENT_ID.value and (OAUTH_CLIENT_SECRET.value or OAUTH_CODE_CHALLENGE_METHOD.value) and OPENID_PROVIDER_URL.value:

        def oidc_oauth_register(oauth: OAuth):
            client_kwargs = {
                "scope": OAUTH_SCOPES.value,
                **({"token_endpoint_auth_method": OAUTH_TOKEN_ENDPOINT_AUTH_METHOD.value} if OAUTH_TOKEN_ENDPOINT_AUTH_METHOD.value else {}),
                **({"timeout": int(OAUTH_TIMEOUT.value)} if OAUTH_TIMEOUT.value else {}),
            }
    ...
    if configured_providers and not OPENID_PROVIDER_URL.value:
        ...
        log.warning("Set OPENID_PROVIDER_URL to your OAuth provider's OpenID Connect discovery endpoint to fix logout functionality.")
...
if CUSTOM_NAME:
    try:
        ...
        if r.ok:
            if "logo" in data:
                WEBUI_FAVICON_URL = url = f"https://api.openwebui.com{data['logo']}" if data["logo"][0] == "/" else data["logo"]
...
USER_PERMISSIONS_WORKSPACE_MODELS_ACCESS = os.environ.get("USER_PERMISSIONS_WORKSPACE_MODELS_ACCESS", "False").lower() == "true"

USER_PERMISSIONS_WORKSPACE_KNOWLEDGE_ACCESS = os.environ.get("USER_PERMISSIONS_WORKSPACE_KNOWLEDGE_ACCESS", "False").lower() == "true"

USER_PERMISSIONS_WORKSPACE_PROMPTS_ACCESS = os.environ.get("USER_PERMISSIONS_WORKSPACE_PROMPTS_ACCESS", "False").lower() == "true"

USER_PERMISSIONS_WORKSPACE_TOOLS_ACCESS = os.environ.get("USER_PERMISSIONS_WORKSPACE_TOOLS_ACCESS", "False").lower() == "true"

USER_PERMISSIONS_WORKSPACE_MODELS_ALLOW_PUBLIC_SHARING = os.environ.get("USER_PERMISSIONS_WORKSPACE_MODELS_ALLOW_PUBLIC_SHARING", "False").lower() == "true"

USER_PERMISSIONS_NOTES_ALLOW_PUBLIC_SHARING = os.environ.get("USER_PERMISSIONS_NOTES_ALLOW_PUBLIC_SHARING", "False").lower() == "true"

USER_PERMISSIONS_WORKSPACE_KNOWLEDGE_ALLOW_PUBLIC_SHARING = os.environ.get("USER_PERMISSIONS_WORKSPACE_KNOWLEDGE_ALLOW_PUBLIC_SHARING", "False").lower() == "true"

USER_PERMISSIONS_WORKSPACE_PROMPTS_ALLOW_PUBLIC_SHARING = os.environ.get("USER_PERMISSIONS_WORKSPACE_PROMPTS_ALLOW_PUBLIC_SHARING", "False").lower() == "true"

USER_PERMISSIONS_WORKSPACE_TOOLS_ALLOW_PUBLIC_SHARING = os.environ.get("USER_PERMISSIONS_WORKSPACE_TOOLS_ALLOW_PUBLIC_SHARING", "False").lower() == "true"
...
USER_PERMISSIONS_CHAT_CONTINUE_RESPONSE = os.environ.get("USER_PERMISSIONS_CHAT_CONTINUE_RESPONSE", "True").lower() == "true"

USER_PERMISSIONS_CHAT_REGENERATE_RESPONSE = os.environ.get("USER_PERMISSIONS_CHAT_REGENERATE_RESPONSE", "True").lower() == "true"
...
USER_PERMISSIONS_CHAT_MULTIPLE_MODELS = os.environ.get("USER_PERMISSIONS_CHAT_MULTIPLE_MODELS", "True").lower() == "true"

USER_PERMISSIONS_CHAT_TEMPORARY = os.environ.get("USER_PERMISSIONS_CHAT_TEMPORARY", "True").lower() == "true"

USER_PERMISSIONS_CHAT_TEMPORARY_ENFORCED = os.environ.get("USER_PERMISSIONS_CHAT_TEMPORARY_ENFORCED", "False").lower() == "true"


USER_PERMISSIONS_FEATURES_DIRECT_TOOL_SERVERS = os.environ.get("USER_PERMISSIONS_FEATURES_DIRECT_TOOL_SERVERS", "False").lower() == "true"

USER_PERMISSIONS_FEATURES_WEB_SEARCH = os.environ.get("USER_PERMISSIONS_FEATURES_WEB_SEARCH", "True").lower() == "true"

USER_PERMISSIONS_FEATURES_IMAGE_GENERATION = os.environ.get("USER_PERMISSIONS_FEATURES_IMAGE_GENERATION", "True").lower() == "true"

USER_PERMISSIONS_FEATURES_CODE_INTERPRETER = os.environ.get("USER_PERMISSIONS_FEATURES_CODE_INTERPRETER", "True").lower() == "true"
...
ENABLE_ADMIN_WORKSPACE_CONTENT_ACCESS = os.environ.get("ENABLE_ADMIN_WORKSPACE_CONTENT_ACCESS", "True").lower() == "true"
...
def validate_cors_origin(origin):
    ...
    if parsed_url.scheme not in schemes:
        raise ValueError(f"Invalid scheme in CORS_ALLOW_ORIGIN: '{origin}'. Only 'http' and 'https' and CORS_ALLOW_CUSTOM_SCHEME are allowed.")
...
Analyze the chat history to determine the necessity of generating search queries, in the given language. By default, **prioritize generating 1-3 broad and relevant search queries** unless it is absolutely certain that no additional information is required.
The aim is to retrieve comprehensive, updated, and valuable information even with minimal uncertainty. If no search is unequivocally needed, return an empty list.
...
CODE_INTERPRETER_BLOCKED_MODULES = [library.strip() for library in os.environ.get("CODE_INTERPRETER_BLOCKED_MODULES", "").split(",") if library.strip()]
...
if VECTOR_DB == "oracle23ai":
    ...
    if ORACLE_DB_USE_WALLET and (not ORACLE_WALLET_DIR or not ORACLE_WALLET_PASSWORD):
        raise ValueError("Oracle23ai requires setting ORACLE_WALLET_DIR and ORACLE_WALLET_PASSWORD when using wallet authentication.")
...
RAG_EMBEDDING_MODEL_AUTO_UPDATE = not OFFLINE_MODE and os.environ.get("RAG_EMBEDDING_MODEL_AUTO_UPDATE", "True").lower() == "true"

RAG_EMBEDDING_MODEL_TRUST_REMOTE_CODE = os.environ.get("RAG_EMBEDDING_MODEL_TRUST_REMOTE_CODE", "True").lower() == "true"
...
RAG_RERANKING_MODEL_AUTO_UPDATE = not OFFLINE_MODE and os.environ.get("RAG_RERANKING_MODEL_AUTO_UPDATE", "True").lower() == "true"

RAG_RERANKING_MODEL_TRUST_REMOTE_CODE = os.environ.get("RAG_RERANKING_MODEL_TRUST_REMOTE_CODE", "True").lower() == "true"
...
AUDIO_STT_SUPPORTED_CONTENT_TYPES = PersistentConfig(
    ...
    [content_type.strip() for content_type in os.environ.get("AUDIO_STT_SUPPORTED_CONTENT_TYPES", "").split(",") if content_type.strip()],
)
