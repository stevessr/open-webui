...
def process_tool_result(
    ...
    elif (tool_type == "external" and isinstance(tool_result, tuple)) or (direct_tool and isinstance(tool_result, list) and len(tool_result) == 2):
...
async def chat_completion_tools_handler(request: Request, body: dict, extra_params: dict, user: UserModel, models, tools) -> tuple[dict, dict]:
    ...
    def get_tools_function_calling_payload(messages, task_model_id, content):
        ...
        chat_history = "\n".join(f'{message["role"].upper()}: """{get_content_from_message(message)}"""' for message in recent_messages)
...
async def chat_completion_files_handler(request: Request, body: dict, extra_params: dict, user: UserModel) -> tuple[dict, dict[str, list]]:
    __event_emitter__ = extra_params["__event_emitter__"]
    sources = []
    ...
    if features:
        ...
        if features.get("code_interpreter"):
            form_data["messages"] = add_or_update_user_message(
                (request.app.state.config.CODE_INTERPRETER_PROMPT_TEMPLATE if request.app.state.config.CODE_INTERPRETER_PROMPT_TEMPLATE != "" else DEFAULT_CODE_INTERPRETER_PROMPT),
                form_data["messages"],
            )
                ...
                try:
                    ...
                    if auth_type == "bearer":
                        ...
                        try:
                            ...
                            oauth_token = await request.app.state.oauth_client_manager.get_oauth_token(user.id, f"mcp:{server_id}")
        ...
        for source in sources:
            if "document" in source:
                for document_text, document_metadata in zip(source["document"], source["metadata"]):
                    ...
                    source_id = document_metadata.get("source", None) or source.get("source", {}).get("id", None) or "N/A"

                    if source_id not in citation_idx_map:
                        citation_idx_map[source_id] = len(citation_idx_map) + 1

                    context_string += f'<source id="{citation_idx_map[source_id]}"' + (f' name="{source_name}"' if source_name else "") + f">{document_text}</source>\n"
...
async def process_chat_response(request, response, form_data, user, metadata, model, events, tasks):
                    ...
                    if tasks.get(TASKS.TAGS_GENERATION):
                        ...
                        if res and isinstance(res, dict):
                            if len(res.get("choices", [])) == 1:
                                ...
                                tags_string = response_message.get("content") or response_message.get("reasoning_content", "")
                            else:
                                tags_string = ""
    ...
    event_emitter = None
    event_caller = None
    if "session_id" in metadata and metadata["session_id"] and "chat_id" in metadata and metadata["chat_id"] and "message_id" in metadata and metadata["message_id"]:
        event_emitter = get_event_emitter(metadata)
    ...
    if not any(content_type in response.headers["Content-Type"] for content_type in ["text/event-stream", "application/x-ndjson"]):
        return response
    ...
    if event_emitter and event_caller:
        ...
        async def response_handler(response, events):
            def serialize_content_blocks(content_blocks, raw=False):
                ...
                for block in content_blocks:
                    if block["type"] == "text":
                            ...
                            for tool_call in tool_calls:
                                ...
                                if tool_result is not None:
                                    ...
                                    tool_calls_display_content = (
                                        ...
                                        + tool_name
                                        + '" arguments="'
                                        + {html.escape(json.dumps(tool_arguments))}
                                        + '" result="'
                                        + {html.escape(json.dumps(tool_result, ensure_ascii=False))}
                                        + '" files="'
                                        + {html.escape(json.dumps(tool_result_files)) if tool_result_files else ""}
                                        + '" embeds="'
                                        + {html.escape(json.dumps(tool_result_embeds))}
                                        + '">\n<summary>Tool Executed</summary>\n</details>\n'
                                    )
                                else:
                                    tool_calls_display_content = tool_calls_display_content + '<details type="tool_calls" done="false" id="' + tool_call_id + '" name="' + tool_name + '" arguments="' + html.escape(json.dumps(tool_arguments)) + '">\n<summary>Executing...</summary>\n</details>\n'
                        ...
                        reasoning_display_content = html.escape("\n".join((f"> {line}" if not line.startswith(">") else line) for line in block["content"].splitlines()))
            ...
            content = message.get("content", "") if message else last_assistant_message if last_assistant_message else ""
                            ...
                            if content_blocks[-1]["attributes"].get("type") == "code":
                                ...
                                if request.app.state.config.CODE_INTERPRETER_ENGINE == "pyodide":
                                    ...
                                    output = await execute_code_jupyter(
                                        ...
                                        code,
                                        (request.app.state.config.CODE_INTERPRETER_JUPYTER_AUTH_TOKEN if request.app.state.config.CODE_INTERPRETER_JUPYTER_AUTH == "token" else None),
                                        (request.app.state.config.CODE_INTERPRETER_JUPYTER_AUTH_PASSWORD if request.app.state.config.CODE_INTERPRETER_JUPYTER_AUTH == "password" else None),
