...
async def get_knowledge(user=Depends(get_verified_user)):
    ...
    if user.role == "admin" and BYPASS_ADMIN_ACCESS_CONTROL:
        ...
        knowledge_bases = await Knowledges.get_knowledge_bases_by_user_id(user.id, "read")
    ...
    for knowledge_base in knowledge_bases:
        ...
        if knowledge_base.data:
            files = await Files.get_file_metadatas_by_ids(knowledge_base.data.get("file_ids", []))
...
async def get_knowledge_list(user=Depends(get_verified_user)):
    ...
    if user.role == "admin" and BYPASS_ADMIN_ACCESS_CONTROL:
        ...
        knowledge_bases = await Knowledges.get_knowledge_bases_by_user_id(user.id, "write")
...
async def create_new_knowledge(request: Request, form_data: KnowledgeForm, user=Depends(get_verified_user)):
    if user.role != "admin" and not await has_permission(user.id, "workspace.knowledge", request.app.state.config.USER_PERMISSIONS):
        raise HTTPException(
...
async def reindex_knowledge_files(request: Request, user=Depends(get_verified_user)):
    ...
    for knowledge_base in knowledge_bases:
        ...
        if not knowledge_base.data or not isinstance(knowledge_base.data, dict):
            log.warning(f"Knowledge base {knowledge_base.id} has no data or invalid data ({knowledge_base.data!r}). Deleting.")
            try:
    ...
    log.info(f"Reindexing completed. Deleted {len(deleted_knowledge_bases)} invalid knowledge bases: {deleted_knowledge_bases}")
    return True
...
async def update_knowledge_by_id(
    ...
    if knowledge.user_id != user.id and not await has_access(user.id, "write", knowledge.access_control) and user.role != "admin":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
