...
async def get_notes(request: Request, user=Depends(get_verified_user)):
    if user.role != "admin" and not await has_permission(user.id, "features.notes", request.app.state.config.USER_PERMISSIONS):
        raise HTTPException(
    ...
    notes = [
        ...
        for note in await Notes.get_notes_by_permission(user.id, "write")
    ]
...
async def get_note_list(request: Request, page: Optional[int] = None, user=Depends(get_verified_user)):
    if user.role != "admin" and not await has_permission(user.id, "features.notes", request.app.state.config.USER_PERMISSIONS):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=ERROR_MESSAGES.UNAUTHORIZED,
    ...
    notes = [NoteTitleIdResponse(**note.model_dump()) for note in await Notes.get_notes_by_permission(user.id, "write", skip=skip, limit=limit)]
...
async def create_new_note(request: Request, form_data: NoteForm, user=Depends(get_verified_user)):
    if user.role != "admin" and not await has_permission(user.id, "features.notes", request.app.state.config.USER_PERMISSIONS):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=ERROR_MESSAGES.UNAUTHORIZED,
...
async def update_note_by_id(request: Request, id: str, form_data: NoteForm, user=Depends(get_verified_user)):
    if user.role != "admin" and not await has_permission(user.id, "features.notes", request.app.state.config.USER_PERMISSIONS):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=ERROR_MESSAGES.UNAUTHORIZED,
    ...
    if user.role != "admin" and (user.id != note.user_id and not await has_access(user.id, type="write", access_control=note.access_control)):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=ERROR_MESSAGES.DEFAULT())
